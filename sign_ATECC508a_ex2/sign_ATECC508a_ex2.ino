...
void setup() {
 ...
 // atecc.createSignature(message); // by default, this uses the private key securely stored and locked in slot 0.
  // this atecc createSignature is weird. It is not coupled to what I have in printMessage.... need to rewrite code?? 
  if(atecc.createSignature(message,0x0000) == false) {
     Serial.println("Failure to generate Signature");
     Serial.println();
     if(atecc.loadTempKey(message) == false) {
        Serial.println("Failure to load temp Key");
        Serial.println();
     }
     if(atecc.signTempKey(0x0000) == false) {
        Serial.println("Failure to sign temp key");
        Serial.println();
     } 
  }

  if(atecc.sendCommand(0x16,NONCE_MODE_PASSTHROUGH,0x0000) == true) {
      Serial.println("With nothing, I passed!");
  }

    if(atecc.sendCommand(0x16,NONCE_MODE_PASSTHROUGH,0x0000,message,32) == true) {
      Serial.println("With 32 bytes, I passed!");
  }

  atecc.loadTempKeyDebug(message);
...
void printInfo()
{
  ...
  if (atecc.configLockStatus && atecc.dataOTPLockStatus && atecc.slot0LockStatus) 
  {
    if(atecc.generatePublicKey(0x0000,true) == false)
    {
